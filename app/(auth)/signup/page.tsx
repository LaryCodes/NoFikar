...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // signUp() returns a user object even when email confirmation is
      // required, but no session exists until the link is confirmed.
      // Only redirect into the authenticated app if a session actually exists.
      if (data.session) {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-primary/5 via-background to-secondary/10">
      <Card className="w-full max-w-md glass-strong">
        <CardHeader className="space-y-3 text-center">
          ...
          <CardDescription className="text-base">
            Join NoFikar and keep your family safe
          </CardDescription>
        </CardHeader>
